    ...
    def on_enter(self):
        ...
        if storage.exists("listas"):
            ...
            for idx, list_name in enumerate(listas_guardadas.keys(), start = 1):
                etiqueta = Label(markup = True, text = f"[b][color=#000000][{idx}] {list_name}[/color][/b]", size_hint_y = None, height = 40)
                self.listas_layout.add_widget(etiqueta)
        else: 
            etiqueta = Label(markup = True, text = f"[b][color=#FF4500]No hay lisats guardadas[/color][/b]", size_hint_y = None, height = 40, font_size = '15sp')
            self.listas_layout.add_widget(etiqueta)
        ...
        if storage.exists("listas"):
            ...
            try: 
                ...
                if seleccion < 0 or seleccion >= len(listas_guardadas):
                    detalles = Label(markup = True, text = f"[b][color=#FF4500]Numero de lista no valido[/color][/b]", size_hint_y = None, height = 40, font_size = '15sp')
                ...
                for nombre, precio in items: 
                    detalles = Label(markup = True, text = f"[b][color=#000000]{nombre} - ${precio:.2f}[/color][/b]", size_hint_y = None, height = 40, color = (0,0,0,1))
                    self.productos_pasados.add_widget(detalles)
            ...
            detalles = Label(markup = True, text = f"[b][color=#FF4500]No hay lisats guardadas[/color][/b]", size_hint_y = None, height = 40, font_size = '15sp')
            self.productos_pasados.add_widget(detalles)
